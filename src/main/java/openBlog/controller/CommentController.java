package openBlog.controller;

import openBlog.model.Comment;
import openBlog.model.Post;
import openBlog.service.*;
import openBlog.utils.JsonUtils;
...
public class CommentController {
	...
	public @ResponseBody String deleteComment(@PathVariable("postId") Long postId,
	...
	public @ResponseBody String editComment(@Valid @ModelAttribute(value = "comment") Comment comment,
			BindingResult result, @PathVariable("postId") Long postId, @PathVariable("commentId") Long commentId) {
		if (result.hasErrors()) {
	...
	@PreAuthorize("hasRole('ROLE_USER')")
	@RequestMapping(value = "/posts/{postId}/comments/{commentId}/source", method = RequestMethod.GET, produces = "text/plain; charset=utf-8")
	public @ResponseBody String getCommentSource(@PathVariable("postId") Long postId,
			@PathVariable("commentId") Long commentId) {
